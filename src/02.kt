private const val INPUT = "515\t912\t619\t2043\t96\t93\t2242\t1385\t2110\t860\t2255\t621\t1480\t118\t1230\t99\n" +
...
fun main(args: Array<String>) {
    ...
    val lines: List<List<Int>> = INPUT.split("\n")
            .map { row: String ->
                row.split("\t")
                        .map { number: String ->
                            number.toInt()
    ...
    fun partOne() {
        print(lines.map { it.sorted() }.sumBy { it.last() - it.first() })
    }

    fun partTwo() {
        var sum = 0
        val l = lines.map { it.sortedDescending() }
